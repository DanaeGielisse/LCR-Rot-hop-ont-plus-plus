...
def lcr_rot(input_fw, input_bw, sen_len_fw, sen_len_bw, target, sen_len_tr, keep_prob1, keep_prob2, l2, _id='all'):
    ...
    outputs_init = tf.concat([outputs_l_init, outputs_r_init, outputs_t_l_init, outputs_t_r_init], 1)
    att_outputs = dot_produce_attention_layer(outputs_init, None, 2 * FLAGS.n_hidden, l2, FLAGS.random_base, 'fin')
    outputs_l = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,0], 2), outputs_l_init))
    outputs_r = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,1], 2), outputs_r_init))
    outputs_t_l = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,2], 2), outputs_t_l_init))
    ...
    for i in range(2):
        ...
        outputs_init = tf.concat([outputs_l_init, outputs_r_init, outputs_t_l_init, outputs_t_r_init], 1)
        att_outputs = dot_produce_attention_layer(outputs_init, None, 2 * FLAGS.n_hidden, l2, FLAGS.random_base, 'fin'+str(i))
        outputs_l = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,0], 2), outputs_l_init))
        outputs_r = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,1], 2), outputs_r_init))
        outputs_t_l = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,2], 2), outputs_t_l_init))
        outputs_t_r = tf.squeeze(tf.matmul(tf.expand_dims(att_outputs[:,:,3], 2), outputs_t_r_init))
